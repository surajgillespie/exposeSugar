define(["webL10n",
    ...
    activity.sampleSystemCalls = function (callback) {
        function onResponseReceived(error, result) {
            //console.log(result);
            if (error === null) {
                ...
                {
                console.log("error occured while making call!!") 
                }
            }
        

        bus.sendMessage("activity.system_call", [], onResponseReceived);
    };

    activity.sugarLogDir = function (callback) {
        function onResponseReceived(error, result) {
            //console.log(result);
            if (error === null) {
                callback(null, result)
        ...
        bus.sendMessage("activity.sugar_logger", [], onResponseReceived);
    };
